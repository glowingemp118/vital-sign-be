...
export class TranscriptionService {
    ...
    async createTranscription(req: any) {
        ...
        const isDoctorExist = await this.userModel.findOne({ _id: new mongoose.Types.ObjectId(doctor), user_type: UserType.Doctor });
                ...
                pipeline.push({
                    $match: {
                        $or: [
                            ...
                            { "voice.transcription": { $regex: `.*${search}.*`, $options: 'i' } },
                            { "voice.latestSummary.summary.riskPatterns": { $regex: search, $options: "i" } }
                        ]
                    }
                })
            }
